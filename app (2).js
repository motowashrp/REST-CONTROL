...
import { requireAuth, logout, formatCOP, timeAgo } from './auth.js';
import {
...
function renderMesas(mesas) {
  ...
  grid.innerHTML = mesas.sort((a, b) => a.numero - b.numero).map(m => {
    ...
    const cls = estilos[m.estado] || estilos.libre;
    ...
    return `<div onclick="accionMesa('${m.id}','${m.estado}')"
      class="border rounded-xl p-4 text-center cursor-pointer transition-all hover:shadow-sm ${cls}">
...
function renderKanban(pedidos) {
  ...
  activos.forEach(p => { if (cols[p.estado]) cols[p.estado].push(p); });
...
window.abrirModalPedido = async () => {
  ...
  if (!mesas.length) { alert('No hay mesas ocupadas. Primero ocupa una mesa desde el módulo Mesas.'); return; }
  ...
  showModal(`
    ...
    <div class="space-y-3">
      <div>
        ...
        <select id="m-mesa" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-1">${opsMesas}</select>
      </div>
      <div>
        <label class="text-xs text-gray-500">Platos</label>
        <div class="mt-1 max-h-48 overflow-y-auto border border-gray-200 rounded-lg p-2">${opsItems || '<p class="text-sm text-gray-400 p-2">No hay platos disponibles.</p>'}</div>
      </div>
      <div>
        <label class="text-xs text-gray-500">Nota especial (opcional)</label>
        <input id="m-nota" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-1" placeholder="Ej: sin cebolla, término del pollo...">
      </div>
    </div>
...
window.guardarPedido = async () => {
  ...
  const items = checks.map(c => ({
    id: c.value, nombre: c.dataset.nombre,
    precio: parseFloat(c.dataset.precio), cantidad: 1
  }));
  ...
  document.getElementById('grid-menu').innerHTML = filtered.map(i => `
    <div class="bg-white border border-gray-100 rounded-xl p-4 hover:shadow-sm transition-shadow">
      ...
      <div class="flex justify-between items-center">
        ...
        <button onclick="toggleDisponible('${i.id}',${i.disponible})" class="text-xs text-gray-400 hover:text-gray-700 transition-colors">
          ${i.disponible ? 'Marcar agotado' : 'Disponible'}
        </button>
      </div>
    </div>`).join('') || '<p class="text-sm text-gray-400 col-span-3">Sin platos en esta categoría.</p>';
}
...
window.abrirModalMenu = () => {
  showModal(`
    ...
    <div class="space-y-3">
      <div><label class="text-xs text-gray-500">Nombre del plato *</label>
      ...
      <div><label class="text-xs text-gray-500">Descripción</label>
        <input id="mn-desc" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-1" placeholder="Ingredientes o descripción breve"></div>
...
function renderCaja(pedidos) {
  ...
  const colores = {
    espera:   'bg-gray-100 text-gray-600',
  ...
  document.getElementById('tabla-inv').innerHTML = inv.map(i => {
    ...
    return `<tr class="border-b border-gray-50 hover:bg-gray-50">
      ...
      <td class="px-4 py-3 text-right">
        <button onclick="editarStock('${i.id}',${i.stock})" class="text-xs text-gray-400 hover:text-gray-700">Editar</button>
      </td>
    </tr>`;
  }).join('') || '<tr><td colspan="6" class="px-4 py-6 text-center text-sm text-gray-400">Sin productos. Agrega el primero.</td></tr>';
}
...
window.abrirModalInventario = () => {
  showModal(`
    <h3 class="text-base font-semibold mb-4">Agregar producto al inventario</h3>
    <div class="space-y-3">
...
function renderReportes(pedidos) {
  ...
  document.getElementById('chart-ventas').innerHTML = ventasDia.map((v, i) =>
    `<div class="flex flex-col items-center gap-1 flex-1">
      <div class="w-full bg-green-500 rounded-t transition-all" style="height:${Math.max(4, Math.round((v / maxV) * 100))}px"></div>
  ...
  document.getElementById('top-platos').innerHTML = top.map(([n, v]) =>
    `<div class="flex items-center gap-3">
      ...
      <div class="flex-1 h-1.5 bg-gray-100 rounded-full">
        <div class="h-1.5 bg-green-500 rounded-full" style="width:${Math.round((v / maxP) * 100)}%"></div>
      </div>
...
function renderUsuarios(users) {
  const colores = {
    ...
    cajero:  'bg-amber-100 text-amber-800',
...
window.abrirModalUsuario = () => {
  showModal(`
    ...
    <div class="space-y-3">
      ...
      <div><label class="text-xs text-gray-500">Correo electrónico *</label>
    ...
    <div class="flex justify-end gap-2 mt-5">
      <button onclick="closeModal()" class="text-sm px-4 py-2 border border-gray-200 rounded-lg">Cancelar</button>
      <button onclick="crearUsuario()" class="text-sm px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">Crear usuario</button>
    </div>`);
};
...
window.crearUsuario = async () => {
  ...
  try {
    ...
    await addDoc(collection(db, 'usuarios'), {
      uid: cred.user.uid, nombre, email, rol, activo: true, creadoEn: serverTimestamp()
    });
    closeModal();
  } catch (e) {
    const msgs = {
